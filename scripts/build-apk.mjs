...
async function main() {
  ...
  // Создаём релизный ключ, если нет
  if (!fs.existsSync(KEYSTORE_PATH)) {
    console.log("Creating release keystore...");
    const javaHome = process.env.JAVA_HOME || "C:\\Program Files\\Eclipse Adoptium\\jdk-17.0.14.7-hotspot";
    execSync(
      `"${javaHome}\\bin\\keytool" -genkey -v -keystore "${KEYSTORE_PATH}" -alias release -keyalg RSA -keysize 2048 -validity 10000 -storepass rustore123 -keypass rustore123 -dname "CN=MBDOU, OU=Admin, O=MBDOU, L=Vyborg, ST=Lenobl, C=RU"`,
      { stdio: "inherit" }
    );
  }
  ...
  const signedApk = path.join(ANDROID_DIR, "app", "build", "outputs", "apk", "release", "app-release-signed.apk");
  const javaHome = process.env.JAVA_HOME || "C:\\Program Files\\Eclipse Adoptium\\jdk-17.0.14.7-hotspot";

  if (fs.existsSync(unsignedApk)) {
    execSync(
      `"${javaHome}\\bin\\apksigner" sign --ks "${KEYSTORE_PATH}" --ks-pass pass:rustore123 --ks-key-alias release --out "${signedApk}" "${unsignedApk}"`,
      { stdio: "inherit" }
    );
